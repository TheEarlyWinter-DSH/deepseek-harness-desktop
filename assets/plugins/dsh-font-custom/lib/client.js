...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    function buildCss(cfg) {
      ...
      if (cfg.widgetColor) css += ":root{--dsh-widget-fg:" + cfg.widgetColor + "}";
      if (cfg.widgetGlowColor) css += ":root{--dsh-widget-glow:" + cfg.widgetGlowColor + "}";
      if (cfg.widgetGlow) {
        css += "@keyframes dshWidgetSweep{0%{background-position:220% 0}100%{background-position:-120% 0}}" +
          // 余额徽章：扫光掠过徽章背景（文本色不变，峰/谷徽章语义色保留）。
          "body[data-dsh-widget-glow=\"1\"] .dsh-balance-dock{" +
          "background-image:linear-gradient(110deg,transparent 38%,var(--dsh-widget-glow,#ffd76b) 50%,transparent 62%);" +
          "background-size:200% 100%;background-repeat:no-repeat;animation:dshWidgetSweep 2.6s linear infinite}" +
          // 高峰提醒弹窗标题：文字流光（currentColor → 流光色 → currentColor）。
          "body[data-dsh-widget-glow=\"1\"] .dspg_title," +
          "body[data-dsh-widget-glow=\"1\"] .__fc_wmock_title{" +
          "background-image:linear-gradient(110deg,currentColor 40%,var(--dsh-widget-glow,#ffd76b) 50%,currentColor 60%);" +
          "background-size:200% 100%;-webkit-background-clip:text;background-clip:text;" +
          "-webkit-text-fill-color:transparent;animation:dshWidgetSweep 2.6s linear infinite}";
      }
      // 文字颜色覆盖（无 !important 不足以越过 dsh-offpeak 自带规则时兜底）：
      // OffPeak 弹窗标题消费 --dsh-widget-fg（未设置时回退其原色）。
      if (cfg.widgetColor) {
        css += ".dspg_title,.__fc_wmock_title{color:var(--dsh-widget-fg,#e6a23c)!important}";
      }
      return css;
    }
    ...
    function applyConfig(cfg) {
      ...
      try {
        if (cfg.widgetGlow) document.body.setAttribute("data-dsh-widget-glow", "1");
        else document.body.removeAttribute("data-dsh-widget-glow");
      } catch (e) { /* body 未就绪时跳过 */ }
    }
