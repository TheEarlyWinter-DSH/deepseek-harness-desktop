...
class SessionWatcher {
  ...
  process(file) {
    ...
    // 若该会话有标准 turn/start / turn/end，则只在 turn/end 时通知；
    // 若会话无 turn 事件（如某些极简/测试流），则回退为按 assistant/message 通知。
    const count = rec.hasTurnEvents ? turnEnds : assistantMessages;
    if (count > 0) {
      this.emit(rec, count);
      return true;
  ...
  emit(rec, count) {
    const h = rec.header || {};
    // 过滤子代理（subagent）会话：dsh 会话模型里根会话 parentId 为 null/空。
    // 子代理的 turn/end 是主任务的中间步骤，不作为面向用户的“任务完成”通知。
    if (h.parentId) return;
    this.onTurnEnd({
      sessionId: h.id,
      title: rec.title || h.title || '会话',
      cwd: h.cwd,
      count,
    });
  }
...
module.exports = { SessionWatcher, scanZstdFrames, decodeFrame, expandRow };
